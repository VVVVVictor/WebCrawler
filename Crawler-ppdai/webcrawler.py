...
def login():
    ...
    try:
        req = urllib2.Request(urlAuth, postdata, headers)
        result = urllib2.urlopen(req)
        result.close()
        #for index, cookie in enumerate(cj):
        #    print '[',index,']',cookie
            
        req2 = urllib2.Request(urlAccount, headers = headers)
...
def analyzeData_ppdai(orderID, webcontent):
    ...
    borrowScore = re.search(u'\d+', tag_borrowScore)
    if borrowScore:
    ...
    if tag_aboutme:
        ...
        #print aboutme
        if aboutme: #部分网页格式错乱 list/451828
    ...
    req_user = urllib2.Request(userurl, None, headers) 
    ...
    while True:
        try:
            ...
            response_user.close()
            #print m_user
            analyzeUserData_ppdai(userName, m_user)
...
def analyzeUserData_ppdai(userID, usercontent):
    ...
    if tag_borrowListArea:
        ...
        for page in range(1, int(pageNumber)+1):
            ...
            req_userPage = urllib2.Request(pageurl, None, headers)
            while True:
                try:
            ...
            if tag_borrowListArea:
                if tag_borrowListArea.find(text=re.compile(u'(\s|\n)*暂时还没有任何借款(\s|\n)*')) is None:
                    ...
                    for borrowItem in tag_borrowList:
                        ...
                        tag_tr2 = tag_tr1.find_next_sibling('tr')#表格第二行，包括信用等级、进度、投标数、时间等
                        #信用等级
...
#enddef analyzeUserData_ppdai
'''http://www.ppdai.com/user/781766877w?page=2'''
#--------------------------------------------------
def getConfig():
